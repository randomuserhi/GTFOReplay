(function () {
    let timelineMap = {
        ...
        "event": function (snapshot, t, lerp) {
            let time = t.time;
            if (RHU.exists(lerp)) {
        ...
        "dynamicProp": function (snapshot, t, lerp) {
            let ev = t.detail;
            dynamicPropMap[ev.type](snapshot, t, ev, lerp);
        },
        "dynamic": function (snapshot, t, lerp) {
            let dynamic = t.detail;
            if (!snapshot.dynamics.has(dynamic.instance)) {
                if (!RHU.exists(lerp) || lerp == 1)
                    throw ReferenceError(`Referenced dynamic does not exist: t: ${t.time} ${dynamic.instance}`);
                return;
            }
    ...
    let eventMap = {
        "unknown": function (snapshot, ev) {
            throw new Error("Unkown event type");
        },
        "playerJoin": function (snapshot, ev, time) {
            let e = ev.detail;
        ...
        "enemyDespawn": function (snapshot, ev) {
            let e = ev.detail;
            snapshot.enemies.delete(e.instance);
            snapshot.dynamics.delete(e.instance);
        },
        "enemyDead": function (snapshot, ev, time) {
            let e = ev.detail;
            ...
            if (RHU.exists(dyn)) {
                ...
                throw new ReferenceError("Referenced enemy does not exist.");
        },
        "enemyBehaviourChangeState": function (snapshot, ev) {
            let e = ev.detail;
            ...
            if (RHU.exists(enemy)) {
                ...
                throw new ReferenceError("Enemy does not exist to change behaviour state of.");
        },
        "enemyLocomotionChangeState": function (snapshot, ev) {
            let e = ev.detail;
            let enemy = snapshot.enemies.get(e.instance);
            if (RHU.exists(enemy)) {
                ...
                throw new ReferenceError("Enemy does not exist to change locomotion state of.");
        },
        "enemyBulletDamage": function (snapshot, ev, time) {
            ...
            if (RHU.exists(enemy) && RHU.exists(eDynamic)) {
                ...
                if (e.sentry) {
                    if (RHU.exists(player.sentry))
                        pDynamic = snapshot.dynamics.get(player.sentry);
                    else
                        throw ReferenceError("Player damaged an enemy with a sentry, yet a sentry was not placed by them.");
                }
                else
                    pDynamic = snapshot.dynamics.get(player.instance);
                ...
                throw new ReferenceError("Enemy does not exist to do bullet damage.");
        },
        "enemyMeleeDamage": function (snapshot, ev, time) {
            let replay = snapshot.owner;
            let e = ev.detail;
            let enemy = snapshot.enemies.get(e.instance);
            let eDynamic = snapshot.dynamics.get(e.instance);
            if (RHU.exists(enemy) && RHU.exists(eDynamic)) {
                ...
                throw new ReferenceError("Enemy does not exist to do melee damage.");
        },
        "enemyMineDamage": function (snapshot, ev, time) {
        },
        "playerDead": function (snapshot, ev, time) {
            let e = ev.detail;
            if (RHU.exists(snapshot.slots[e.slot])) {
                ...
                if (RHU.exists(dyn)) {
                    ...
                    throw new ReferenceError("Referenced player does not exist.");
                player.alive = false;
            }
            else
                throw new ReferenceError("Player that died does not exist");
        },
        "playerRevive": function (snapshot, ev) {
            let e = ev.detail;
            if (RHU.exists(snapshot.slots[e.slot]) && RHU.exists(snapshot.slots[e.source])) {
                ...
                throw new ReferenceError("Either target or source of revive did not exist.");
        },
        "playerTongueDamage": function (snapshot, ev, time) {
            let e = ev.detail;
            if (RHU.exists(snapshot.slots[e.slot]) && snapshot.enemies.has(e.source)) {
                ...
                throw new ReferenceError("Either enemy or player did not exist.");
        },
        "playerMeleeDamage": function (snapshot, ev, time) {
            let e = ev.detail;
            if (RHU.exists(snapshot.slots[e.slot]) && snapshot.enemies.has(e.source)) {
                let player = snapshot.snet.get(snapshot.slots[e.slot]);
                let pDynamic = snapshot.dynamics.get(player.instance);
        ...
        "playerPelletDamage": function (snapshot, ev, time) {
        },
        "playerBulletDamage": function (snapshot, ev, time) {
            let e = ev.detail;
            if (RHU.exists(snapshot.slots[e.slot]) && RHU.exists(snapshot.slots[e.source])) {
                let target = snapshot.snet.get(snapshot.slots[e.slot]);
        ...
        "playerMineDamage": function (snapshot, ev, time) {
        },
        "playerFallDamage": function (snapshot, ev, time) {
        },
        "playerPelletDodge": function (snapshot, ev, time) {
        },
        "playerTongueDodge": function (snapshot, ev, time) {
        },
        "playerWield": function (snapshot, ev) {
            let e = ev.detail;
            if (RHU.exists(snapshot.slots[e.slot])) {
                let player = snapshot.snet.get(snapshot.slots[e.slot]);
                ...
                throw new ReferenceError("Player did not exist.");
        },
        "doorChangeState": function (snapshot, ev) {
            let e = ev.detail;
            snapshot.doors.set(e.id, e.state);
        },
        "doorDamage": function (snapshot, ev) {
            let e = ev.detail;
        },
        "spawnMine": function (snapshot, ev) {
            let e = ev.detail;
            let owner = snapshot.slots[e.slot];
            if (!RHU.exists(owner))
                throw ReferenceError("Player that owns this mine does not exist.");
        ...
        "despawnMine": function (snapshot, ev) {
            let e = ev.detail;
            snapshot.mines.delete(e.instance);
        },
        "explodeMine": function (snapshot, ev, time) {
            let e = ev.detail;
            if (e.slot !== 255) {
                if (RHU.exists(snapshot.slots[e.slot]) && snapshot.mines.has(e.instance)) {
                    ...
                    throw new ReferenceError("Either mine or player did not exist.");
            }
            snapshot.mines.delete(e.instance);
        },
        "tripline": function (snapshot, ev) {
            let e = ev.detail;
            if (snapshot.mines.has(e.instance)) {
                snapshot.mines.get(e.instance).length = e.length * GTFOReplaySettings.scale;
            }
            else
                throw ReferenceError("Mine does not exist.");
        },
        "spawnSentry": function (snapshot, ev, time) {
            let e = ev.detail;
            let owner = snapshot.slots[e.slot];
            if (!RHU.exists(owner))
                throw ReferenceError("Player that owns this sentry does not exist.");
            let player = snapshot.snet.get(owner);
            if (player.sentry != null)
                throw Error("Player has already spawned a sentry.");
        ...
        "despawnSentry": function (snapshot, ev) {
            ...
            if (snapshot.sentries.has(e.instance)) {
                ...
                throw ReferenceError("Sentry does not exist.");
        },
        "spawnPellet": function (snapshot, ev, time) {
            let e = ev.detail;
        ...
        "despawnPellet": function (snapshot, ev, time) {
            let e = ev.detail;
        ...
        "spawnTongue": function (snapshot, ev) {
            let e = ev.detail;
            snapshot.tongues.set(e.instance, new GTFOTongue(e.instance, e.dimensionIndex));
        },
        "despawnTongue": function (snapshot, ev) {
            let e = ev.detail;
            if (snapshot.tongues.has(e.instance)) {
                snapshot.tongues.delete(e.instance);
            }
            else
                throw ReferenceError("tongue does not exist.");
        },
        "setTongue": function (snapshot, ev) {
            throw new Error("Unreachable");
        },
        "spawnGlue": function (snapshot, ev, time) {
            let e = ev.detail;
        ...
        "despawnGlue": function (snapshot, ev) {
            let e = ev.detail;
            if (snapshot.glue.has(e.instance)) {
                ...
                throw ReferenceError("glue does not exist.");
        },
        "enemyAlerted": function (snapshot, ev, time) {
            let e = ev.detail;
            if (snapshot.enemies.has(e.instance)) {
                ...
                throw ReferenceError("enemy does not exist.");
        },
        "enemyScreamed": function (snapshot, ev, time) {
            let e = ev.detail;
            if (snapshot.enemies.has(e.instance)) {
                let enemy = snapshot.enemies.get(e.instance);
        ...
        "enemyTargetSet": function (snapshot, ev, time) {
            let e = ev.detail;
            if (snapshot.enemies.has(e.instance)) {
                let enemy = snapshot.enemies.get(e.instance);
                if (e.slot != 255) {
        ...
        "bulletShot": function (snapshot, ev, time) {
            let e = ev.detail;
            snapshot.tracers.push({
    ...
    let dynamicPropMap = {
        "unknown": function (snapshot, t, ev) {
            throw new Error("Unkown event type");
        },
        "tongue": function (snapshot, t, ev, lerp) {
            let _tongue = ev.detail;
            if (!snapshot.tongues.has(_tongue.instance)) {
                if (!RHU.exists(lerp) || lerp == 1)
                    throw ReferenceError(`Referenced tongue does not exist: t: ${t.time} ${_tongue.instance}`);
                return;
            }
    ...
    let GTFOSnapshot = window.GTFOSnapshot = function (owner, tick, index, time, parser) {
        this.owner = owner;
    ...
    GTFOSnapshot.prototype.lerp = function (time, t) {
    ...
    GTFOSnapshot.prototype.clone = function (snapshot) {
        ...
        this.mines = new Map();
        for (let kv of snapshot.mines)
            this.mines.set(kv[0], kv[1]);
        this.pellets = new Set();
        for (let v of snapshot.pellets)
            this.pellets.add(v);
        this.glue = new Set();
        for (let v of snapshot.glue)
            this.glue.add(v);
        this.tongues = new Map();
        for (let kv of snapshot.tongues)
            this.tongues.set(kv[0], GTFOTongue.clone(kv[1]));
        this.sentries = new Map();
        for (let kv of snapshot.sentries)
            this.sentries.set(kv[0], kv[1]);
        this.trails = new Map();
        ...
        this.enemies = new Map();
        for (let kv of snapshot.enemies)
            this.enemies.set(kv[0], GTFOEnemy.clone(kv[1]));
        this.doors = new Map();
        for (let kv of snapshot.doors)
            this.doors.set(kv[0], kv[1]);
        this.snet = new Map();
        for (let kv of snapshot.snet)
            this.snet.set(kv[0], GTFOPlayer.clone(kv[1]));
        this.slots = new Array(4);
        for (let i = 0; i < 4; ++i)
            this.slots[i] = snapshot.slots[i];
        this.players = new Map();
        for (let kv of snapshot.players)
            this.players.set(kv[0], kv[1]);
        this.dynamics = new Map();
        for (let kv of snapshot.dynamics)
            this.dynamics.set(kv[0], {
    ...
    GTFOSnapshot.clone = function (snapshot) {
