RHU.import(RHU.module({ trace: new Error(),
    ...
    callback: function () {
        ...
        replay.prototype.update = function (t) {
            let delta = (t - this.prev) * this.timescale;
            this.prev = t;
            if (this.play && this.time <= this.replay.timeline[this.replay.timeline.length - 1].time)
                this.time += delta;
            ...
            for (let tracer of tracers) {
                if (this.visualTracers) {
                    if (tracer.type == "damage")
                        continue;
                }
                else {
                    if (tracer.type == "visual")
                        continue;
                }
            ...
            for (let kv of dynamics) {
                ...
                if (snapshot.pellets.has(instance)) {
                    ...
                    if (snapshot.players.has(instance)) {
                        ...
                        let polygon = function (ctx, radius, sides) {
                            if (sides < 3)
                                return;
                            ctx.save();
