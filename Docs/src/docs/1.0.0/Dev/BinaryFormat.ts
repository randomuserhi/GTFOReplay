RHU.require(new Error(), { 
    ...
    docs.jit = () => docuscript<RHUDocuscript.Language, RHUDocuscript.FuncMap>((lib, include) => {
        ...
        br();
        p(
            "However, this replay mod packs quaternions into a smaller datastructure by utilising the fact that normalized quaternions always have a magnitude of 1. This way",
            " the largest component can be omitted and recalculated by subtracting all the existing components from 1. We store which component of the quaternion is missing by using a single byte:"
        );
        img("QuaternionPacked.png", "80%");
        p(
            "Above, the index ", ic([], "i"), " represents the missing component and ", ic([], "a,b,c"), " represent the remaining components of the quarternion."
        );
        br();
        p(
        ...
        p(
            ...
            cb(["csharp"], `[MethodImpl(MethodImplOptions.AggressiveInlining)]
...
public static ushort FloatToHalf(float x) {
    ...
    return (ushort)((b & 0x80000000) >> 16 | Convert.ToInt32(e > 112) * ((((e - 112) << 10) & 0x7C00) | m >> 13) | (Convert.ToInt32(e < 113) & Convert.ToInt32(e > 101)) * ((((0x007FF000 + m) >> (int)(125 - e)) + 1) >> 1) | Convert.ToUInt32(e > 143) * 0x7FFF); // sign : normalized : denormalized : saturate
        ...
        h1("Structure");
        p(
            "The basic structure of the replay file consists of the map followed by stored snapshots taken by the game:"
        );
