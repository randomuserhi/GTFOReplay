RHU.require(new Error(), { 
    ...
    docs.jit = () => docuscript<RHUDocuscript.Language, RHUDocuscript.FuncMap>((lib, include) => {
        ...
        ul(
            ...
            frag(ic([], `Space`), " - Pause the replay."),
            frag(ic([], `Mouse Scroll`), " - Zoom in/out."),
            frag(ic([], `Left Mouse Drag`), " - Panning the camera.")
        );
        ...
        ul(
            "Visual shots are purely aesthetic and are non-representative of whether a player hit an enemy or not. They are the stored rendered shots the game shows you from your teammates.",
            frag("By setting this to ", ic([], "false"), " you can see exactly what players hit and exact damage events recorded, but you cannot see missed shots etc..."),
            i("NOTE: If you are not hosting the lobby, you cannot utilise non-visual tracers as these can only be recorded by host.")
        );
...
console.log(totalDamage);`);
        ...
        p(
            "This is a bug that may occure if the replay fails to add the ", ic([], "despawnPellet"), " event. To get around this, prior loading your replay, type the following into DevConsole:"
        );
        cb(["javascript"], `GTFOReplaySettings.maxProjectileLifetime = 5000;`);
        p(
            "This will enforce that projectiles can only exist for 5 seconds."
        );
    }, rhuDocuscript);
});