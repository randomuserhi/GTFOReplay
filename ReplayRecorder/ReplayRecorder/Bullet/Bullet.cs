...
namespace ReplayRecorder.Bullets {
    internal class rBullet : ISerializable {
        ...
        public rBullet(uint id, float damage, bool hit, byte dimension, Vector3 start, Vector3 end) {
            this.id = id;
            this.start = start;
            this.end = end;
            this.damage = damage;
        ...
        public const int SizeOf = 2 + BitHelper.SizeOfHalf + BitHelper.SizeOfVector3 * 2;
        ...
        public void Serialize(FileStream fs) {
            ...
            BitHelper.WriteBytes(start, buffer, ref index);
            BitHelper.WriteBytes(end, buffer, ref index);
            fs.Write(buffer);
        }
    }

    internal class Bullet {
        public static void OnBulletShot(uint id, float damage, bool hit, byte dimensionIndex, Vector3 start, Vector3 end) {
            APILogger.Debug($"Bullet shot {damage} ({dimensionIndex}) [{start.x}, {start.y}, {end.z}] -> [{end.x}, {end.y}, {end.z}]");
            SnapshotManager.AddEvent(GameplayEvent.Type.BulletShot, new rBullet(id, damage, hit, dimensionIndex, start, end));
        }
    }
}
