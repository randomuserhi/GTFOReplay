RHU.module(new Error(), "components/organisms/docpages", {
...
}, function ({ Macro, style, filterlist, rhuDocuscriptStyle, rhuDocuscriptPages, docs, indices, }) {
    ...
    const loadPage = (versionStr, directory, callback) => {
        if (!directory.page)
            throw new Error("No page for this directory.");
    ...
    const loadAll = (versionStr) => {
        const version = docs.get(versionStr);
        if (version) {
            version.walk((dir) => {
                const page = dir;
                if (page.page) {
                    loadPage(versionStr, page);
                }
            });
        }
    };
    ...
    const docpages = Macro((() => {
        ...
        docpages.prototype.render = function (page, index, directory, scrollTop = true) {
            ...
            let i = 0;
            let scrollTarget;
            const [pageDom, destructor] = docuscript.render(page, {
                pre: (node) => {
                    if (node.__type__ === "h") {
                        const h = node;
                        const _i = i.toString();
                ...
                post: (node, dom) => {
                    if (node.__type__ === "h") {
                        ...
                        h.onclick = () => {
                            ...
                            document.documentElement.scroll(0, dom.offsetTop - document.documentElement.offsetTop -
