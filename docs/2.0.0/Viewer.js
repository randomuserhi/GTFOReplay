RHU.require(new Error(), {
    ...
    docs.jit = () => docuscript((lib, include) => {
        const { p, h1, h2, ul, frag, i, img } = lib;
        ...
        h1("Camera Controls");
        ul(frag(ic([], `1-4`), " - Follow the player in slot 1-4."), frag(ic([], `WASD`), " - Classic first person movement."), frag(ic([], `Space`), `/`, ic([], `Cntrl`), " - Up or down along the Y-axis."), frag(ic([], `Scroll Wheel`), " - Increases or decreases the camera's movement speed."), frag(ic([], `Tab`), " - Shows the scoreboard."));
        h1("Time Controls");
        ul(frag(ic([], `Up`), `/`, ic([], `Down`), " - Skip 10 seconds."), frag(ic([], `Left`), `/`, ic([], `Right`), " - Skip 5 seconds."), frag(ic([], `F`), " - Pause the replay."));
        h1("Information");
