RHU.require(new Error(), {
    ...
    docs.jit = () => docuscript((lib, include) => {
        const { p, br, link, h1, h2, ul, frag, img, pl, i } = lib;
        ...
        img("demo.gif");
        h1("Installation");
        ul(frag("Follow the installation process for Bepinex on ", link("https://gtfo.thunderstore.io/package/BepInEx/BepInExPack_GTFO/", "thunderstore"), "."), frag("Download the Bepinex plugin dll and viewer from ", link("https://github.com/randomuserhi/GTFOReplay/releases", "here"), " under ", ic([], "Assets"), "."), frag("Drag the plugin dll into your bepinex plugin folder."), frag("Alternatively, a mod manager such as ", link("https://thunderstore.io/c/gtfo/p/ebkr/r2modman/", "r2modman"), " can be used. The viewer must, however, still be downloaded from the above github link."));
        h1("Usage");
        ul(frag(p("While you are playing a match, the game will write a replay file (e.g ", ic([], "R1A1 2024-11-10 22-10.gtfo"), ") to your game folder. Typically at ", ic([], "steamapps/common/GTFO"), "."), img("replay_location.png")), frag(p("You can load the replay in the viewer by opening ", ic([], "viewer.exe"), " choosing the replay file.")), frag(p("Load your replay file using the button on the top left or the 'Load Replay' button when the viewer is first opened. Any errors that occur will show up in the dev console accessible via ", ic([], "Cntrl+Shift+I"), ". Feel free to message ", ic([], "@randomuserhi"), " on discord regarding these errors."), img("viewer_load.png", "85%")), frag("For information about the viewer, visit notes on viewer usage ", pl(["Viewer"], "here"), "."));
        ...
        h1("Documentation");
        p("(Unfinished) Documentation can be found ", link("https://randomuserhi.gitbook.io/gtfo-replay", "here"), ".");
    }, rhuDocuscript);
});
